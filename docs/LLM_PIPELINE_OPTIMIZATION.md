...
Детальное логирование кешированных токенов реализовано через `src/utils/token_cache_logger.py`:

```python
# Автоматическое логирование после каждого LLM вызова
...
```
[Two-Stage: Stage 1 Extraction] Токены: prompt=21854, completion=3701, total=25555
[Two-Stage: Stage 1 Extraction] 💰 Кешировано: 15000 токенов (68.6% от prompt)
[Two-Stage: Stage 1 Extraction] 💵 Экономия: $0.0375 (50.0%) | Стоимость: $0.0375 вместо $0.0750
```

### Поддерживаемые провайдеры и модели

| Провайдер | Модель | Скидка | Метод | Мин. размер |
|-----------|--------|--------|-------|-------------|
| OpenAI | gpt-4o | 50% | Автоматический | 1024 токена |
| OpenAI | gpt-4o-mini | 50% | Автоматический | 1024 токена |
| OpenAI | gpt-4-turbo | 50% | Автоматический | 1024 токена |
| Anthropic | claude-3-5-sonnet | 90% | Явный (cache_control) | 1024 токена |
| Anthropic | claude-3-opus | 90% | Явный (cache_control) | 1024 токена |

### Пример экономии на реальных данных

**Встреча 30 минут, транскрипция 20K токенов:**

```
Stage 1 (Extraction):
  Prompt: 21,854 токенов
  Кешировано: 15,000 токенов (68.6%)
  Экономия: $0.0375 (50%)

Stage 2 (Reflection):
  Prompt: 13,783 токенов  
  Кешировано: 10,000 токенов (72.5%)
  Экономия: $0.0250 (50%)

Итого экономия: $0.0625 (~44% от общей стоимости)
```
...
```bash
# Включить prompt caching (по умолчанию True)
ENABLE_PROMPT_CACHING=true

# Логировать метрики кеширования
LOG_CACHE_METRICS=true

# Минимальная длина транскрипции для кеширования (~1024 токенов)
MIN_TRANSCRIPTION_LENGTH_FOR_CACHE=3000

# Показывать метрики в финальном сообщении (отладка)
CACHE_METRICS_IN_FINAL_MESSAGE=false
```

### Как это работает

**OpenAI (автоматическое):**
```python
# Префиксы >= 1024 токенов автоматически кешируются
messages = [
    {"role": "system", "content": system_prompt},  # Кешируется
    {"role": "user", "content": [
        {"type": "text", "text": f"ТРАНСКРИПЦИЯ:\n\n{transcription}"},  # Кешируется
        {"type": "text", "text": task_prompt}  # Не кешируется (изменяется)
    ]}
]
```
...
**Anthropic (явное):**
```python
# Требует явных cache_control маркеров
system = [
    {"type": "text", "text": system_prompt},
    {
        "type": "text",
        "text": f"ТРАНСКРИПЦИЯ:\n\n{transcription}",
        "cache_control": {"type": "ephemeral"}  # Явная метка
    }
]
```
...
1. **Структура промптов**: Статический контент (system + транскрипция) вначале, изменяющийся в конце
2. **Минимальный размер**: Убедитесь что префикс >= 1024 токенов (~3000 символов)
3. **Группировка запросов**: Stage 1 и Stage 2 с одной транскрипцией выполняйте подряд (кеш живет 5-10 минут)
4. **Мониторинг**: Регулярно проверяйте cache hit rate в логах

**Подробнее см. [PROMPT_CACHING_GUIDE.md](./PROMPT_CACHING_GUIDE.md)**
