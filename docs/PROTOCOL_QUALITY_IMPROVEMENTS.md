# 🎯 Улучшения качества протоколов - Реализовано
...
- ⚠️ Увеличение времени обработки на 20-30%
...
1. **Время**: +20-30% времени обработки
2. **Стоимость**: Больше запросов к LLM API
3. **Провайдеры**: Пока только OpenAI
4. **Минимальная длина**: Не имеет смысла для встреч < 30 минут
