...
- 💰 Экономия токенов LLM: **~3,500-4,500 токенов** на запрос
  - ~3,000-4,000 токенов от фрагментов речи
  - ~200-500 токенов от удаления секции "ИНФОРМАЦИЯ О СПИКЕРАХ"
- ⚡ Экономия CPU: **~1-2 мс** на извлечение фрагментов
- 📦 Уменьшение размера промпта: **~12-15 KB** меньше
- 🎯 Секция "ИНФОРМАЦИЯ О СПИКЕРАХ ИЗ ДИАРИЗАЦИИ" **полностью отсутствует** в промпте

**При `FULL_TEXT_MATCHING=false` (по умолчанию):**
- Поведение остается **прежним**
- Фрагменты извлекаются и используются как обычно
- Секция со спикерами отображается с фрагментами речи
