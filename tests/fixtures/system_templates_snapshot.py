...
Осознанное изменение (критика v8, 2026-07-24): из «Стандартного протокола» и
«Краткого резюме» удалена секция next_steps — в живых прогонах она пересказывала
«Задачи и сроки» парафразом; сроки живут внутри задач.

Осознанное изменение (критика v12, 2026-08-17): «Краткому резюме» возвращена
секция risks_and_blockers (риски проступали внутри «Ключевых выводов» в 13
протоколах из 19), а участники в его шапке склеиваются в одну строку — столбик
имён занимал 56-69% первого мобильного экрана.
"""
...
{% endif %}"""
    ...
    # Единственная запись с r"""…""": инлайн-шапка несёт литерал \n в
    # Jinja-выражении split('\n') — обычная строка превратила бы его в перевод
    # строки и сломала выражение.
    'Краткое резюме встречи': (
        r"""# {{ meeting_title or 'Резюме встречи' }}

{% if date %}**Дата:** {{ date }}{% if time %} · {{ time }}{% endif %}
{% endif %}{% if participants %}**👥 Участники:** {{ participants.split('\n') | map('trim') | select | join(', ') }}
{% endif %}
{% if decisions %}
## ✅ Решения
{{ decisions }}
