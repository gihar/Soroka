...
@pytest.mark.asyncio
async def test_resume_tail_with_none_meeting_type_and_empty_list(monkeypatch):
    """Возобновление с meeting_type=None и пустым списком не падает: ЭТАП 1
    определяет тип на генерации (v5 skip не сломан), протокол собирается."""
    import types
    ...
    service.llm_gen = types.SimpleNamespace(
        optimized_llm_generation=AsyncMock(
            return_value={"meeting_title": "Встреча", "_meeting_type": "general"}
        ),
    ...
    request = ProcessingRequest(
        file_name="a.mp3", llm_provider="openai", user_id=42,
        participants_list=[],
        speaker_mapping={"SPEAKER_1": "Мария Сидорова"},
    )

    result = await service._finalize_protocol(
        request,
        TranscriptionResult(transcription="текст"),
        types.SimpleNamespace(name="Стандартный"),
        types.SimpleNamespace(total_duration=1.0, formatting_duration=0.0),
        meeting_type=None,
    )
