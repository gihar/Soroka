...
def _build_system_prompt(
    ...
    if unified_mode:
        ...
        return base_prompt + (
            ...
            "{\n"
            '  "completeness": 0.0-1.0,     // Полнота извлечения информации\n'
            '  "missing_info": [],          // Список того, что не удалось извлечь\n'
            '  "ambiguous_points": [],      // Неоднозначные моменты\n'
            '  "quality_concerns": []       // Проблемы качества данных\n'
            "}\n\n"
            ...
            "{\n"
            '  "date": "20 октября 2024",  ❌ protocol_data не на верхнем уровне\n'
            '  "protocol_data": {\n'
...
{{
  "completeness": <число 0.0-1.0>,        // Полнота извлечения: насколько полно удалось извлечь информацию
  "missing_info": [<список строк>],       // Что не удалось извлечь или осталось неопределенным
  "ambiguous_points": [<список строк>],   // Моменты, которые можно интерпретировать по-разному
  "quality_concerns": [<список строк>]    // Проблемы качества данных (шум, противоречия, неясность)
}}
...
def _build_segment_analysis_prompt(
    ...
    if speaker_mapping:
        ...
        participants_info += "🎯 ПОШАГОВЫЙ АЛГОРИТМ СОПОСТАВЛЕНИЯ (ПРИОРИТЕТЫ):\n\n"
        
        participants_info += "ПРИОРИТЕТ 1 - ПРЯМЫЕ ПРЕДСТАВЛЕНИЯ:\n"
        participants_info += "   Если SPEAKER_N говорит о себе:\n"
        participants_info += "   • \"Меня зовут Иван\" → SPEAKER_N = Иван\n"
        participants_info += "   • \"Я — Мария Петрова\" → SPEAKER_N = Мария Петрова\n"
        participants_info += "   • \"Это Алексей\" (в начале) → SPEAKER_N = Алексей\n\n"
        
        participants_info += "ПРИОРИТЕТ 2 - ОБРАЩЕНИЯ С ОТВЕТОМ:\n"
        participants_info += "   Анализируй последовательность реплик:\n"
        participants_info += "   • SPEAKER_1: \"Света, как думаешь?\"\n"
        participants_info += "     SPEAKER_2: \"Да, согласна...\"\n"
        participants_info += "     → SPEAKER_2 = Света (обращение + немедленный ответ)\n\n"
        participants_info += "   • SPEAKER_2: \"Да, Алексей, согласен с тобой\"\n"
        participants_info += "     → SPEAKER_1 (предыдущий говоривший) = Алексей\n\n"
        
        participants_info += "ПРИОРИТЕТ 3 - ПОДТВЕРЖДЕНИЕ ЧУЖИХ СЛОВ:\n"
        participants_info += "   Ищи отсылки к предыдущим высказываниям:\n"
        ...
        participants_info += "ПРИОРИТЕТ 4 - УПОМИНАНИЯ С КОНТЕКСТОМ:\n"
        participants_info += "   Упоминания третьих лиц требуют осторожности:\n"
        participants_info += "   • SPEAKER_1: \"мы со Светой обсуждали...\"\n"
        participants_info += "     → Света может быть другой SPEAKER или отсутствовать\n"
        participants_info += "     → Добавляй в mapping ТОЛЬКО если Света действительно говорит\n\n"
        
        participants_info += "📊 КОНТЕКСТНЫЙ АНАЛИЗ ДИАЛОГА:\n\n"
        participants_info += "1. Читай транскрипцию ПОСЛЕДОВАТЕЛЬНО, отслеживай смену SPEAKER_N\n"
        participants_info += "2. При обращении по имени → анализируй кто отвечает следующим\n"
        participants_info += "3. При фразах \"как сказал X\" → ищи в предыдущих репликах\n"
        participants_info += "4. Сохраняй гипотезы и ПРОВЕРЯЙ их по всей транскрипции\n"
        participants_info += "5. Если имя упоминается несколько раз в разных контекстах → повышай уверенность\n\n"
        
        participants_info += "💡 ПРИМЕРЫ ИЗ РЕАЛЬНЫХ ДИАЛОГОВ:\n\n"
        
        participants_info += "ПРИМЕР 1 - Обращение + продолжение:\n"
        participants_info += "   SPEAKER_1: \"Здесь наверное хорошо послушать коллегу из ОРТ. Подожди, я тебя\"\n"
        participants_info += "   SPEAKER_2: \"поправлю, не все тонкости раскрыл...\"\n"
        ...
        participants_info += "   Анализ: Света упомянута как соавтор, но НЕ говорит в этот момент\n"
        participants_info += "   Действие: НЕ добавляй Света в mapping пока она не заговорит или к ней не обратятся\n\n"
        
        participants_info += "ПРИМЕР 4 - Прямое обращение:\n"
        participants_info += "   SPEAKER_1: \"Галя, как ты считаешь?\"\n"
        participants_info += "   SPEAKER_3: \"Я думаю, что нужно...\"\n"
        participants_info += "   Анализ: Прямое обращение по имени с немедленным ответом\n"
        participants_info += "   Действие: SPEAKER_3 = Галина (высокая уверенность)\n\n"
        
        participants_info += "🔍 ОБЯЗАТЕЛЬНЫЙ АНАЛИЗ ПЕРЕД ОТВЕТОМ:\n\n"
        participants_info += "Шаг 1: Перечисли ВСЕ упоминания имен в транскрипции\n"
        participants_info += "Шаг 2: Для каждого имени определи КОНТЕКСТ:\n"
        participants_info += "        - Представление (говорит о себе)\n"
        participants_info += "        - Обращение (к нему обращаются)\n"
        participants_info += "        - Упоминание (говорят о нем)\n"
        participants_info += "Шаг 3: Сопоставь имена с SPEAKER_N используя ПРИОРИТЕТЫ выше\n"
        participants_info += "Шаг 4: Проверь гипотезы на ПРОТИВОРЕЧИЯ по всей транскрипции\n"
        participants_info += "Шаг 5: Верни ТОЛЬКО уверенные сопоставления (уверенность > 70%)\n\n"
        
        participants_info += "📝 ФОРМАТ ИМЕН:\n"
        participants_info += "   • Предпочтительно: 'Имя Фамилия' (БЕЗ отчества)\n"
        participants_info += "   • Преобразуй уменьшительные: Света→Светлана, Леша→Алексей, Володя→Владимир\n"
        ...
        participants_info += "   ❌ НЕ дублируй: если Марат = SPEAKER_2, добавь только один раз\n"
        participants_info += "   ❌ НЕ добавляй имена из упоминаний без подтверждения их участия\n\n"
        
        participants_info += "💡 ПРИМЕРЫ ПРАВИЛЬНЫХ ОТВЕТОВ:\n\n"
        
        participants_info += "Пример А - Есть уверенное сопоставление:\n"
        participants_info += "{\n"
        participants_info += "  \"segment_data\": {\n"
        participants_info += "    \"participants\": \"SPEAKER_1\\nSPEAKER_2\\nSPEAKER_3\",\n"
        participants_info += "    \"decisions\": \"- SPEAKER_1 предложил использовать марки КНДМ\"\n"
        participants_info += "  },\n"
...
ИСПОЛЬЗОВАНИЕ ПРЕДОСТАВЛЕННЫХ ДАННЫХ О ВСТРЕЧЕ:

⚠️ КРИТИЧЕСКИ ВАЖНО:
1. Если в контексте выше предоставлена ИНФОРМАЦИЯ О ВСТРЕЧЕ (тема, дата, время):
   → Используй эти ТОЧНЫЕ данные в соответствующих полях финального протокола
   → Поля: "date", "time" (если есть в template_variables)

2. Если в контексте выше предоставлен ПОЛНЫЙ СПИСОК УЧАСТНИКОВ:
   → Используй ВЕСЬ этот список целиком в поле "participants"
   → НЕ пытайся извлекать участников из текста сегментов
   → НЕ добавляй/удаляй участников из списка
   → Формат: каждое имя с новой строки "Имя Фамилия\\nИмя Фамилия\\n...", БЕЗ ролей
   → Сохрани порядок из предоставленного списка
   
3. Если список участников НЕ предоставлен:
   → Только тогда извлекай участников из текста сегментов (см. правила ниже)
...
- Для участников: если предоставлен ПОЛНЫЙ СПИСОК УЧАСТНИКОВ выше - используй его целиком; если списка нет - объедини всех упомянутых из сегментов, каждое имя с новой строки БЕЗ ролей
...
☝️ ВНИМАНИЕ: Поле "participants" должно содержать ВЕСЬ предоставленный список участников, если он есть в контексте!
...
"""
    ...
    # Логирование кешированных токенов для Chain-of-Thought Segment
    if settings.log_cache_metrics:
        log_cached_tokens_usage(
            response=response,
            context=f"Chain-of-Thought: Segment {segment_idx + 1}/{total_segments}",
            model_name=selected_model,
            provider="openai"
        )
    ...
    **kwargs
    ...
    if provider_name == "openai":
        ...
        try:
            ...
            # Логирование кешированных токенов для Chain-of-Thought Synthesis
            if settings.log_cache_metrics:
                log_cached_tokens_usage(
                    response=response_synthesis,
                    context="Chain-of-Thought: Synthesis",
                    model_name=selected_model,
                    provider="openai"
                )
