...
class MessageBuilder:
    ...
    def templates_help_message(cls) -> str:
        ...
        """
        return (
            "<b>Управление шаблонами протоколов</b>\n\n"
            ...
            "• <code>{{ meeting_title }}</code> - название встречи\n"
            "• <code>{{ date }}</code> / <code>{{ time }}</code> - дата и время\n"
            "• <code>{{ participants }}</code> - список участников\n"
            "• <code>{{ decisions }}</code> - принятые решения\n"
            "• <code>{{ action_items }}</code> - задачи и сроки\n"
            "• <code>{{ risks_and_blockers }}</code> - блокеры и риски\n"
            "• <code>{{ discussion }}</code> - ход обсуждения\n"
            "• <code>{{ next_steps }}</code> - следующие шаги\n\n"

            "<b>Условные секции:</b>\n"
            "Оборачивайте секцию в <code>{% if переменная %}</code> ... "
            "<code>{% endif %}</code> - "
            ...
            "<b>Советы:</b>\n"
            "• Используйте Markdown: <code>#</code> заголовки, <code>-</code> "
            "списки, <code>**жирный**</code>\n"
