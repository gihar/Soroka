...
1. Название темы (краткое и точное) - обязательное поле
2. Описание (что конкретно обсуждалось)
3. Ключевые моменты обсуждения (главные идеи) - массив строк
4. Участники обсуждения (используй ID спикеров из транскрипции: SPEAKER_1, SPEAKER_2 и т.д.) - массив строк
5. Временные метки: start_time, end_time, duration в секундах (если можно определить из транскрипции)
6. Общий тон обсуждения: sentiment (positive/neutral/negative или пустая строка)
...
ВАЖНО:
- title - обязательное поле, остальные поля опциональные
- Если временные метки неизвестны, используй 0.0
- Если список пустой (participants, key_points), используй пустой массив []
- Если значение неизвестно (sentiment), используй пустую строку ""
- extraction_confidence - оценка уверенности в извлечении (0.0-1.0)"""
...
1. Текст решения (что именно решили) - обязательное поле
2. Контекст (почему это решение было принято)
3. Кто принял решение (ID спикеров: SPEAKER_1, SPEAKER_2 и т.д.) - массив строк decision_makers
4. Упомянутые спикеры - массив строк mentioned_speakers
5. Приоритет: high/medium/low
6. Временная метка в секундах (timestamp) - если можно определить из транскрипции
...
ВАЖНО:
- text - обязательное поле, остальные поля опциональные
- Если список пустой (decision_makers, mentioned_speakers), используй пустой массив []
- Если временная метка неизвестна, используй 0.0
- Если значение неизвестно (context, priority), используй пустую строку "" или "medium" для priority
- extraction_confidence - оценка уверенности в извлечении (0.0-1.0)"""
...
1. Описание задачи (что нужно сделать) - обязательное поле
2. Ответственный (ID спикера: SPEAKER_1, SPEAKER_2 и т.д.) - assignee
3. Имя ответственного (assignee_name) - всегда пустая строка, имена определяются отдельно
4. Срок выполнения (deadline) - строка с текстом или пустая строка если не указан
5. Приоритет: critical/high/medium/low
6. Контекст задачи
7. Временная метка в секундах (timestamp) - если можно определить из транскрипции
...
ВАЖНО:
- description - обязательное поле, остальные поля опциональные
- Все строковые поля (assignee, assignee_name, deadline, priority, context) должны быть строками, НЕ null
- Если значение неизвестно, используй пустую строку "" (не null)
- assignee_name всегда пустая строка "" - имена определяются отдельно на основе маппинга спикеров
- Если временная метка неизвестна, используй 0.0
