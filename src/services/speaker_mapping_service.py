...
КРИТЕРИИ ДЛЯ СОПОСТАВЛЕНИЯ (В ПОРЯДКЕ ПРИОРИТЕТА):
1. ПРЕДСТАВЛЕНИЯ (наивысший приоритет): Кто представляется в начале? ("Меня зовут...", "Я — ...")
2. ОБРАЩЕНИЯ (высокий приоритет): Как участники обращаются друг к другу? ("Иван, как думаешь?", "Света, расскажи...")
3. РОЛИ И ПОВЕДЕНИЕ (средний приоритет): Сопоставь стиль речи с ролью участника (см. "типичное поведение")
4. СТИЛЬ РЕЧИ (средний приоритет): Авторитетный/директивный vs исполнительский/вопросительный
5. КОНТЕКСТ (низкий приоритет): Упоминания должностей, отделов, ответственности, экспертизы

⚠️ ВАЖНО: Используй критерии сверху вниз — если есть прямое представление или обращение, это надежнее, чем догадки по стилю речи!

ТИПИЧНЫЕ ПАТТЕРНЫ ИДЕНТИФИКАЦИИ:

ОРГАНИЗАТОР/РУКОВОДИТЕЛЬ: "Давайте начнем", "Переходим к следующему", задает вопросы участникам, принимает решения, распределяет задачи

ДОКЛАДЧИК/ИСПОЛНИТЕЛЬ: "Я сделал...", "У меня готово...", отчитывается о работе, объясняет детали своей области

ЭКСПЕРТ/КОНСУЛЬТАНТ: "На мой взгляд", "Я рекомендую...", дает советы, использует профессиональную терминологию

ВАЖНО — ПРАВИЛА СОПОСТАВЛЕНИЯ:
...
{{
  ...
  "mapping_notes": "Подробные объяснения для каждого сопоставления с указанием использованных сигналов"
}}
...
- Формат: "SPEAKER_N: [объяснение]. SPEAKER_M: [объяснение]."
...
{{
  ...
  "unmapped_speakers": ["SPEAKER_3"],
  "mapping_notes": "SPEAKER_1: представился в начале ('Меня зовут Иван'), ведет встречу, задает вопросы всем участникам, принимает решения — четко соответствует роли Менеджер проектов. SPEAKER_2: к ней обращаются 'Мария', отчитывается о технических задачах, использует термины разработки — соответствует роли Разработчик. SPEAKER_3: говорит мало, нет явных сигналов (ни представления, ни обращений), стиль речи нейтральный — уверенность недостаточна для сопоставления."
}}

ПРИМЕР 2 (пограничный случай с порогом 0.7):
{{
  "speaker_mappings": {{
    "SPEAKER_1": "Иван Петров"
  }},
  ...
  "unmapped_speakers": ["SPEAKER_2", "SPEAKER_3"],
  "mapping_notes": "SPEAKER_1: четко представился и ведет встречу, координирует участников, принимает решения — высокая уверенность (0.90). SPEAKER_2: есть косвенные признаки (упоминается имя 'Мария' один раз), но нет прямого представления или множественных обращений — уверенность 0.65 < 0.7, поэтому НЕ включен в маппинг. SPEAKER_3: очень мало реплик, нет идентифицирующих признаков, невозможно сопоставить с участником — уверенность 0.50."
}}
