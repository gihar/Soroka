...
def setup_callback_handlers(user_service, template_service, processing_service) -> Router:
    """Aggregate all callback routers into one."""
    router = Router()
    router.include_router(setup_template_callbacks(user_service, template_service, processing_service))
