...
def strip_emoji(text: str) -> str:
    """Снять эмодзи из строки протокола — единый проход для документных каналов.

    Живёт здесь, а не в рендерере: эмодзи снимают оба файловых канала. В PDF —
    потому что в TTF-шрифтах нет глифов (вместо ✅ был бы тофу), в Word — потому
    что глиф там как раз есть, и цветная картинка в заголовке делает документ,
    который пересылают «наверх», похожим на чат (ADR-0009). Чат и `.md`
    эмодзи-метки сохраняют: там они работают навигацией.
    """
