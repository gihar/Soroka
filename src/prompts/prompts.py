...
Конкретные, измеримые действия с относительными сроками ("к концу недели", "к следующей встрече").""",
    ...
    "yesterday_progress": """yesterday_progress — Что сделано с прошлой встречи (дейли: «вчера»).
Формат: "**Имя Фамилия**:\\n- сделанное". Только завершённая или продвинутая работа. Планы — в today_plans.""",

    "today_plans": """today_plans — Планы участников на сегодня (дейли: «сегодня»).
Формат: "**Имя Фамилия**:\\n- план". Только заявленные планы на ближайший день. Сделанное — в yesterday_progress.""",

    "went_well": """went_well — Что прошло хорошо за спринт (ретроспектива).
Только позитив: удачные практики, успехи, что стоит сохранить. Проблемы — в to_improve.""",

    "to_improve": """to_improve — Что стоит улучшить (ретроспектива).
Проблемы процесса и предложения по улучшению. Формулируй конструктивно, без обвинений. Успехи — в went_well.""",

    "lecturer": """lecturer — Лектор/докладчик выступления.
Формат: "Имя Фамилия". Только ведущий(е), не слушатели. Если не определить — оставь пустым.""",
...
Формат: "**Имя Фамилия**: тема и ключевые тезисы". Если не было — оставь пустым.""",

    "controversial_points": """controversial_points — Спорные моменты и разногласия.
Позиции сторон с атрибуцией, аргументы и итог. Если не было — оставь пустым.""",
...
Формат: "Имя → Имя: отзыв". Если не было — оставь пустым.""",

    "individual_reflections": """individual_reflections — Личные выводы участников ("я понял, что...").
Формат: "**Имя Фамилия**: рефлексия". Если не было — оставь пустым.""",

    "platform": """platform — Платформа проведения (Zoom, Teams, офлайн).
Простой текст. Если не упомянута — оставь пустым.""",
...
def build_generation_system_prompt(
    ...
    base = (
        ...
        "- Списки: через \\n с маркером '- '\n"
        "- Если данных для поля нет — верни пустую строку \"\". Не придумывай текст-заглушку\n\n"
