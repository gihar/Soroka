...
FIELD_SPECIFIC_RULES: Dict[str, str] = {
    "meeting_title": """meeting_title — Название встречи: 3-7 слов о её сути.
Формат: "Планёрка команды продукта", "Согласование бюджета Q3". Извлекай из темы обсуждения или повестки. Без кавычек, без слова "протокол", без даты (для неё есть поле date). Голые аббревиатуры раскрывай, если расшифровка звучит на встрече — заголовок должен быть понятен внешнему адресату, который на встрече не был. Если тему не определить — оставь пустым.""",

    "participants": """participants — Список участников встречи.
Формат: "Имя Фамилия\\nИмя Фамилия". Каждое имя простой строкой, без маркеров списка ("- "). Извлекай из представлений, обращений, упоминаний. Игнорируй третьих лиц, не присутствующих на встрече. Если имя не определить — пиши "Участник N" (по номеру метки SPEAKER_N), а не саму метку.""",
...
Формат: НУМЕРОВАННЫЙ список. "N. Пункт повестки". Извлекай пункты повестки, озвученные в начале встречи. Спонтанные темы — это для discussion.""",
...
Формат: НУМЕРОВАННЫЙ список. "N. Вывод". Каждый пункт — самостоятельный и информативный. Только additive: НЕ повторяй Решения и Задачи — ни дословно, ни пересказом другими словами; вывод несёт только то, чего нет в других секциях (контекст, значимые цифры, риски, инсайты).""",

    "decisions": """decisions — ТОЛЬКО окончательно принятые и согласованные решения.
Формат: НУМЕРОВАННЫЙ список. "N. Решение — хвост". Само решение — первой короткой фразой (примерно до 100 знаков); условия и инициатор — коротким хвостом после тире, НЕ длиннее самого решения. Атрибуцию пиши человеческим оборотом ("предложила Елена, согласовано всеми"), а не телеграфом "Имя: глагол; Имя: глагол". Условие вплетай в текст ("если получим бюджет — ..."), без ярлыка "условие:". Отбирай лишь то, что участники утвердили как итог; просто обсуждавшееся или предложенное сюда НЕ включай. Не предваряй пункты статус-ярлыками ("Решено:", "Обсуждалось:") — заголовок секции уже говорит, что это решения.
Отделяй решение от задачи: если пункт можно исполнить и назначить ему ответственного ("организовать встречу", "отправить запись", "согласовать макеты") — это задача, её место в action_items, а сюда она НЕ дублируется. Решение — про сделанный выбор, договорённость или утверждённый принцип, а не про будущее действие.""",

    "tasks": """tasks — Задачи и поручения со встречи.
Формат: "- Описание задачи — Отв.: Имя Фамилия". Одна работа — ОДИН пункт: нескольких исполнителей перечисли через запятую ("Отв.: Илья, Мария"), НЕ дублируй пункт под каждого. Ответственного указывай ТОЛЬКО если явно назван или понятен из контекста; несопоставленную метку ("Участник 5") ответственным не назначай — пиши "Отв.: уточнить" или опусти ответственного.""",

    "tasks_od": """tasks_od — Поручения руководителей (OD).
Формат: НУМЕРОВАННЫЙ список. "N. Название — описание. Отв.: Имя Фамилия. Срок — DD.MM.YYYY". Если срока нет — опусти. Одна работа — ОДИН пункт: нескольких ответственных перечисли через запятую ("Отв.: Илья, Мария"), НЕ дублируй пункт под каждого. Несопоставленную метку ("Участник 5") ответственным не назначай — пиши "Отв.: уточнить" или опусти ответственного.""",

    "action_items": """action_items — Конкретные действия с ответственными.
Формат: НУМЕРОВАННЫЙ список. "N. Описание задачи — Отв.: Имя Фамилия". БЕЗ роли в скобках! Если срок упомянут — включи в описание. Одна работа — ОДИН пункт: нескольких исполнителей перечисли через запятую ("Отв.: Илья, Мария"), НЕ дублируй пункт под каждого. Несопоставленную метку ("Участник 5") ответственным не назначай — пиши "Отв.: уточнить" или опусти ответственного.""",

    "next_steps": """next_steps — Согласованные следующие шаги после встречи.
Формат: НУМЕРОВАННЫЙ список. "N. Шаг". Конкретные, измеримые действия с относительными сроками ("к концу недели", "к следующей встрече"). Только additive: НЕ повторяй Решения и Задачи — ни дословно, ни пересказом другими словами.""",
...
Формат: НУМЕРОВАННЫЙ список. "N. Что удалось". Только позитив: удачные практики, успехи, что стоит сохранить. Проблемы — в to_improve.""",

    "to_improve": """to_improve — Что стоит улучшить (ретроспектива).
Формат: НУМЕРОВАННЫЙ список. "N. Что улучшить". Проблемы процесса и предложения по улучшению. Формулируй конструктивно, без обвинений. Успехи — в went_well.""",
...
Формат: НУМЕРОВАННЫЙ список. "N. **Риск/Блокер**: описание и план митигации". Ярлык типа обязателен и выделен жирным двойными звёздами: "N. **Риск**: …" или "N. **Блокер**: …". Различай риски (потенциальные) и блокеры (текущие).""",
...
Формат: НУМЕРОВАННЫЙ список. "N. Проблема". Только технические вопросы. Нетехнические проблемы — в issues.""",

    "architecture_decisions": """architecture_decisions — Архитектурные решения с обоснованием.
Формат: НУМЕРОВАННЫЙ список. "N. Решение. Обоснование: почему". Включай отвергнутые альтернативы.""",

    "technical_tasks": """technical_tasks — Технические задачи (разработка, настройка, оптимизация).
Формат: НУМЕРОВАННЫЙ список. "N. Задача". Используй технические термины точно как в транскрипции. Общие задачи — в tasks.""",

    "next_sprint_plans": """next_sprint_plans — Задачи и цели следующего спринта.
Формат: НУМЕРОВАННЫЙ список. "N. Задача с приоритетом". Долгосрочные планы без привязки к спринту не включай.""",
...
Формат: НУМЕРОВАННЫЙ список. "N. Вопрос". Вопросы, требующие проработки или уточнения. Уже отвеченные не включай.""",
...
Организационные объявления, изменения расписания, контекст. "Ловушка" для важного без другого места. Это финал документа: завершай существом (принятый вариант, его эффект, следующая контрольная точка), а не перечнем ограничений системы; ограничение не ставь последним пунктом.""",
}
...
def build_generation_system_prompt(
    ...
    base = (
        ...
        "- Маркированные списки: каждый пункт с новой строки с маркером '- '\n"
        "- Нумерованные списки (правило поля требует \"N. ...\"): каждый пункт "
        "с новой строки как '1. ', '2. ' — БЕЗ маркера '- ' перед номером\n"
        ...
        "- Явные фонетические искажения известных терминов от распознавания речи "
        "нормализуй к канону (например, «котовер» → cutover, «биг дата» → BigData, "
        "«сап» → SAP). Мусор, который не удаётся распознать, не выдавай за термин в "
        "кавычках. Ничего не выдумывай — правь только очевидные искажения\n"
        "- Букву «ё» используй последовательно во всём документе (отчёт, а не отчет); "
        "написание имён собственных не меняй\n\n"
...
def _get_type_specific_instructions(meeting_type: str) -> str:
    ...
    instructions = {
        ...
        'educational': (
            ...
            "- discussion — конспект материала по темам, без атрибуции реплик "
            "и диалогового формата"
        ),
        'brainstorm': (
        ...
        'management': (
            "СПЕЦИФИКА УПРАВЛЕНЧЕСКИХ СОВЕЩАНИЙ:\n"
            "- Каждое поручение: формулировка, ответственный, срок\n"
            "- Директивные решения дословно, с указанием, кто принял\n"
            "- Статус исполнения ранее данных поручений\n"
            "- Вопросы, вынесенные на контроль или следующее совещание"
        ),
    }
