...
class ProtocolGenerator:
    ...
    def _require_provider_key(self, preset: Optional[Dict[str, Any]]) -> None:
        ...
        Уже готовый клиент снимает вопрос: раз идти есть чем, требовать ключ
        незачем. В проде ``default_client`` и строится-то только при заданном
        ключе, так что послабления там нет — зато клиент, подставленный извне
        (тесты, свой транспорт), снова работает. Строить клиента здесь нельзя:
        SDK без ключа бросит своё исключение и подменит внятную ошибку
        настройки невнятной.
        """
